...
uint8_t usi_i2c_slave_internal_address;
uint8_t usi_i2c_slave_address;
uint8_t usi_i2c_mode;
...
uint8_t  USI_Slave_internal_address_set = 0;
...
void USI_I2C_Init(uint8_t address)
{
    ...
    usi_i2c_slave_internal_address=0x00;

	USI_SET_BOTH_INPUT();
    USI_SET_SCL_OUTPUT();
	
	USICR = (1 << USISIE) | (0 << USIOIE) | (1 << USIWM1) | (1 << USIWM0) | (1 << USICS1) | (0 << USICS0) | (0 << USICLK) | (0 << USITC);
